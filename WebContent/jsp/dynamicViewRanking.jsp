...
%>
		<h2><a href="/matoso/servlet/LoadTournament?tournament-id=<%=tournament.getId()%>"><%=tournament.getName()%></a></h2>
		<h3>Session <%=session.getAttribute(SessionCst.ATTR_LAST_PLAYED_SESSION)%></h3>
		<display:table name="sessionScope.ranking" pagesize="<%=nbPerPage%>" id="tableDynamicRanking">
...
%>
		...
		<display:table name="sessionScope.rankingTeam" pagesize="<%=nbPerPage%>" id="tableDynamicRanking">
...
String url;
if (request.getParameter("team") == null) {
	if (displayRankingTeam && nextPage > Math.ceil((double) nbPlayers / nbPerPage))
		url = "/matoso/jsp/dynamicViewRanking.jsp?" + paramName + "=1&team=1";
	else if (nextPage <= Math.ceil((double) nbPlayers / nbPerPage))
		url = "/matoso/jsp/dynamicViewRanking.jsp?" + paramName + "=" + nextPage;
	else
		url = "/matoso/jsp/dynamicViewRanking.jsp?" + paramName + "=1";
} else if (nextPage > Math.ceil((double) nbTeams / nbPerPage))
	url = "/matoso/jsp/dynamicViewRanking.jsp?" + paramName + "=" + 1;
else
