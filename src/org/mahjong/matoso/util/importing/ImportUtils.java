...
public class ImportUtils {
	 ...
	 * Read a XLS file.
	 * @param is
	 * @param spreadSheet spread sheet number to parse (start from 0)
	...
	public static XLSFile readXLS(InputStream is, int spreadSheet) throws ImportXLSException {
		...
		try {
			...
			if(wb == null) {
				LOGGER.warn("Found an input stream that produced a null HSSFWorkbook, returning null.");
				return null;
			}
			...
			LOGGER.info("Sheet 0 \"" + spreadsheetName + "\" has " + rows
					+ " row(s).");
			...
			if(rows != 0) {
				...
				for (int r = 0; r < rows; r++) {
					...
					LOGGER.info("ROW " + row.getRowNum() + " has " + cells
							+ " cell(s).");
	...
	public static List<String[]> readCSV(InputStream is) throws ImportCSVException {
		...
		try {
			...
			CSVReader csvReader = new CSVReader(new InputStreamReader(is), CSV_SEPARATOR, CSV_QUOTE, 1);
